...
public class PanelContlloler : MonoBehaviour
{
    ...
    void Update()
    {
        if (result == false)
        {
            ...
            //この行の下に傾きのプログラムを書いてね
            rotation = tilt * 30f;//傾きを増加させる
            if (-60f > rotation)//もし角度が-60以下なら
            {

                rotation = -60f;//傾きを-60にする
            }
            if (60f < rotation)//もし角度が60以上なら
            {
                rotation = 60f;//傾きを60にする
            }
            transform.rotation = Quaternion.Euler(0f, 0f, rotation);//Z軸をrotationに設定
           }
    }
